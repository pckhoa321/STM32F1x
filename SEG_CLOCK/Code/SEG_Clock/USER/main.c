...
void SEG_control(void){
	...
	//cau hinh nut nhan
	seg_control.GPIO_Mode = GPIO_Mode_IPU;
	seg_control.GPIO_Pin = GPIO_Pin_3;
	GPIO_Init(GPIOB, &seg_control);
	//GPIOB->ODR = 0xffff;
}
...
void timer_Init(void){
	...
	timer_init.TIM_Prescaler = 800 - 1;
...
void check_paused(void){
	if(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_3) == 0){ //button actived
		if(isPaused == 0) //dang chay
			isPaused = 1; //dung
		else
			isPaused = 0; //chay
	}
